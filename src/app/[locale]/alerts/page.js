    ...
    <Layout>
      <div className="p-6 min-h-screen bg-background text-(--card-text)">
        ...
        <section className="mb-10">
          <h2 className="text-lg font-medium mb-4 text-(--muted-text)">{t('parameter-alerts')}</h2>
          <div className="rounded-lg border border-(--border-color) bg-(--card-bg) overflow-hidden">
            {parameterAlerts.length > 0 ? (
              ...
              <p className="p-6 text-center text-(--faint-text) text-sm">{t('no-parameter-alerts')}</p>
            )}
          </div>
        </section>
        ...
        {/* Device Alerts */}
        <section>
          <h2 className="text-lg font-medium mb-4 text-(--muted-text)">{t('device-alerts')}</h2>
          <div className="rounded-lg border border-(--border-color) bg-(--card-bg) overflow-hidden">
            {deviceAlerts.length > 0 ? (
              ...
              <p className="p-6 text-center text-(--faint-text) text-sm">{t('no-device-alerts')}</p>
            )}
          </div>
        </section>
      </div>
