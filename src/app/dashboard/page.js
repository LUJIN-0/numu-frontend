'use client'

import Layout from "@/layout/Layout";
import StatCard from "./components/StatCard";
import Alerts from "./components/Alerts";
import Chart from "./components/Chart";
import LocationMap from "./components/LocationMap";
import CropInfo from "./components/CropInfo";
import GrowthTimeline from "./components/GrowthTimeline";
import { Thermometer, Droplet, AirVent } from "lucide-react";

export default function DashboardPage() {
  return (
    <Layout>
      <div className="flex min-h-screen" style={{ backgroundColor: "var(--background)", color: "var(--foreground)", }}>
        {/* Main Content */}
        <div className="flex-1 flex flex-col">
          <main className="p-6 grid grid-cols-12 gap-4">
            <StatCard
              title="Temperature"
              value="18°C"
              max={100}
              icon={<Thermometer size={16} style={{ color: "var(--muted-text)" }} />}
            />
            <StatCard
              title="Humidity"
              value="70% r.H"
              max={100}
              icon={<Droplet size={16} style={{ color: "var(--muted-text)" }} />}
            />
            <StatCard
              title="AQI"
              value="32"
              max={100}
              icon={<AirVent size={16} style={{ color: "var(--muted-text)" }} />}
            />

            {/* Chart */}
            <div className="col-span-8 rounded-lg p-4" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)", }}>
              <Chart />
            </div>

            {/* Alerts */}
            <div className="col-span-4 rounded-lg p-4" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)", }}>
              <Alerts />
            </div>

            {/* Location */}
            <div className="col-span-4 rounded-lg p-4" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)", }}>
              <LocationMap />
            </div>

            {/* Crop Info */}
            <div className="col-span-4 rounded-lg p-4" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)", }}>
              <CropInfo />
            </div>

            {/* Growth */}
            <div className="col-span-4 rounded-lg p-4" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)", }}>
              <GrowthTimeline />
            </div>
          </main>
        </div>
      </div>
    </Layout>
  );
}
